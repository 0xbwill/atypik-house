...
<body style="margin: 0; width: 100%; padding: 0; -webkit-font-smoothing: antialiased; word-break: break-word">
  <div role="article" aria-roledescription="email" aria-label="New payment host" lang="en">
    <div style="margin: 32px auto; max-width: 336px; border-radius: 8px; background-color: #fff; padding: 24px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -2px rgba(0, 0, 0, 0.1)">
      <h1 style="text-align: center; font-size: 24px; font-weight: 700; color: #2563eb">Confirmation de paiement pour votre logement</h1>
      <p style="margin-top: 16px; font-size: 18px; color: #374151">Bonjour {{{ $reservation->logement->user->name }}},</p>
      <p style="margin-top: 8px; color: #4b5563">Le client <strong>{{{ $reservation->user->name }}}</strong> a effectué le
        paiement pour la réservation de votre logement <strong>{{{ $reservation->logement->title }}}</strong>.</p>
